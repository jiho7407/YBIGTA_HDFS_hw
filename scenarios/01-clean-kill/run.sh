...
# Learning objective:
# Verify that a graceful stop of the Active NameNode triggers automatic
...
usage() {
  cat <<'USAGE'
Usage: scenarios/01-clean-kill/run.sh

Stops the current Active NameNode with docker compose stop, waits for the
Standby NameNode to become writable, restarts the stopped NameNode, then runs
the common validator. Logs go to stderr. JSON goes to stdout.
USAGE
}
...
emit_json() {
  ...
  printf '"name":"Active NameNode clean kill",'
...
main() {
  ...
  emit_json "${passed}" "${active_before}" "${active_after}" "${STOPPED_NN}" "${failure_ms}" "${recovered_ms}" "${recovery_time_ms}" "${write_latency_ms}" "${common_passed}"
